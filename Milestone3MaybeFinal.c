...
void printCacheSimulationResults(CacheStats stats, CacheResults result, CacheCalculations cacheCalc, CacheInput input);
...
int main(int argc, char* argv[]) {
    ...
    printCacheSimulationResults(vmRes.cacheStats, result, cacheCalc, input);
...
CacheResults calculateCacheResults(CacheStats stats, int totalBlocks, int blockSize, int overheadSizeBytes, double implementationMemoryKB, double cacheCost) {
	...
	// this guard is to prevent overflow
    if (stats.blocksEverUsed >= totalBlocks) {
        result.unusedBlocks = 0;
    } else {
...
void printCacheSimulationResults(CacheStats stats, CacheResults result, CacheCalculations cacheCalc, CacheInput input ) {
	...
	printf( "ERROR: Cannot open Test Output File\n" );
	return;

    }

    // fprints are for automated testing
